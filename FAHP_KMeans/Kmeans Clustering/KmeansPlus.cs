...
namespace Fahp_cbr_app
{
    class KmeansPlus
    {
        ...
        begin: PointClusters cluster = new PointClusters();
            ...
            {
                foreach(Case centroid in cluster.PC.Keys)//current centers
                {
                    found = true;
                    foreach (Feature f in centroid.GetFeatures())
        ...
        private List<Case> GetSeedPoints(List<Case> allPoints, int k)
        {
           ...
           /* int count=0;
            foreach (Case c in seedPoints)
            {   
                c.GetFeature("id").SetFeatureValue(count);
                c.GetFeature("cluster").SetFeatureValue("-1");
                count++;
            }*/
            return seedPoints;
        }
        ...
        public int FindCluster_Customize(Case a, List<Case> centroids) // looking for the appropriate cluster for a point 
        {
            ...
            double min = GetEuclideanD(a, centroids[0]); // calculate the distance to the centroid of the first 
            int i = 0;
            int l = 0;
            foreach (Case p in centroids) // looping through all the centroids 
            {
                double d = GetEuclideanD(a, p); // calculate the distance to the centroid of the current 
                if (d < min) // if it is less than before, it's new to kondidat cluster to this point 
                {
                    min = d; // disimilarty smaller
                    l = i;
                }
                i++;
