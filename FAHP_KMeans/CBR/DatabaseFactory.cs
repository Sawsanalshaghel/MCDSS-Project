...
using System;
	...
	{
		public static IDb newInstance(DataSource dataSource)
		{
			int dbType = dataSource.GetDbType();

			if (dbType == DbType.TYPE_ACCESS)
			{
				return new AccessDatabase(dataSource);
			}
			else if (dbType == DbType.TYPE_MSSQL)
			{
				return new MssqlDatabase(dataSource);
			}
			else if (dbType == DbType.TYPE_MYSQL)
			{
				return new MysqlDatabase(dataSource);
			}
			else if (dbType == DbType.TYPE_ORACLE)
			{
